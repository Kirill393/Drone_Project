    ...
    try {
        ...
        if (dsat != 1 || dexp != 1 || dhue != 0) {
            if (img.channels() >= 3)
            {
                ...
                {
                    cv::Mat tmp;
                    cvtColor(sized, tmp, cv::COLOR_RGBA2RGB);    // RGBa to HSV
                    cvtColor(tmp, hsv_src, cv::COLOR_RGB2HSV);
                }
                else
                {
                ...
                if(img.channels() == 4)
                {
                    ...
                    cvtColor(hsv_src, tmp, cv::COLOR_HSV2RGB);
                    cv::split(sized, rgba);
                    cv::split(tmp, rgb);
                ...
                {
                    cvtColor(hsv_src, sized, cv::COLOR_HSV2RGB);    // RGB to HSV
                }
